...
| `trajectory_contrastive_loss_and_logits(contexts, targets, teacher_steps=None)` | Main training forward. Student path is unchanged; detached teacher path may use fewer outer steps when `teacher_steps` is set. |
